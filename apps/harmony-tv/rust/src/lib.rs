/// N-API bridge for HarmonyOS — exposes opentivi-core to ArkTS.
///
/// HarmonyOS supports Node-API (N-API) for native modules. This crate
/// compiles to `libopentivi.so` and is loaded by ArkTS via the standard
/// native-module mechanism.
///
/// The API surface mirrors the UniFFI exports from the Android bridge
/// (`apps/android-tv/rust/src/lib.rs`) but uses `napi-derive` macros
/// instead of UniFFI scaffolding.
...
// SAFETY: CoreContext and the tokio Runtime are both Send+Sync.
